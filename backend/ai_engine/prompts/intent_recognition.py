...
ALARM PARAMETERS:
- title: The name/title of the alarm (required) - extract from phrases like "called X", "named X", "for X"
- alarm_times: Array of times in HH:MM format (required) - convert "7 AM" to "07:00", "3 PM" to "15:00"
- description: Optional description of the alarm - extract from "with description X" or "description X"
- is_snoozable: Boolean, defaults to true

EXAMPLES:
- "Set an alarm for 7 AM called Wake up" → title: "Wake up", alarm_times: ["07:00"]
- "Create an alarm for 8:30 AM named Morning Alarm" → title: "Morning Alarm", alarm_times: ["08:30"]
- "Set an alarm for 6 AM called Gym with description Workout reminder" → title: "Gym", alarm_times: ["06:00"], description: "Workout reminder"
...
3. For time: convert natural language to HH:MM format
4. For description: look for "with description X" or "description X" patterns
5. If you're not confident about the intent (confidence < 0.7), set intent to "unknown"
6. DO NOT guess or use default values for required parameters"""

    # Fallback prompts for different attempt levels
    FALLBACK_PROMPTS = {
        1: """I'm having trouble understanding your request. Let me try a different approach.
...
Please respond with the number or describe what you want in simple terms.""",
        ...
        3: """I apologize, but I'm still having difficulty understanding your request. 
