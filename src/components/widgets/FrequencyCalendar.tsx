...
const FrequencyCalendar = ({ frequency, pillarColor = '#3B82F6' }: FrequencyCalendarProps) => {
  ...
  const getBiasedFrequency = (date: Date): number => {
    const baseFrequency = getFrequencyFromSlider();
    
    // Check if this is the last day of the current period
    let isLastDayOfPeriod = false;
    
    switch (frequency.frequencyPeriod) {
      case 'WEEKLY':
        // Last day of week is Sunday (day 7)
        isLastDayOfPeriod = date.getDay() === 0; // Sunday
        break;
      
      case 'MONTHLY':
        // Last day of month
        const lastDayOfMonth = new Date(date.getFullYear(), date.getMonth() + 1, 0).getDate();
        isLastDayOfPeriod = date.getDate() === lastDayOfMonth;
        break;
      
      case 'YEARLY':
        // Last day of year (December 31st)
        isLastDayOfPeriod = date.getMonth() === 11 && date.getDate() === 31;
        break;
             ...
             {weeks.map((week, weekIndex) => (
               <div key={weekIndex} className="flex flex-col space-y-1">
                 ...
                 return (
                   <div
                     key={dayIndex}
                     className={`w-3 h-3 rounded-sm transition-all ${heatmapColor} ${
