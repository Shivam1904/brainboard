...
import { getCurrentTimeInZone, getZoneTime } from './dateUtils';
...
export const getNextAlarmTime = (alarm_times?: string[]): string => {
    ...
    const nextAlarmTime = alarm_times?.find(time => {
        ...
        const alarmDate = getCurrentTimeInZone();
...
export const checkAlarmTrigger = (
    ...
    alarm_times.forEach((alarmTime: string) => {
        ...
        const alarmStart = getCurrentTimeInZone();
        ...
        if (now >= alarmStart && now < alarmEnd) {
            ...
            const relevantActivities = activityHistory.filter((activity: AlarmActivity) => {
                const t = getZoneTime(new Date(activity.timestamp));
            ...
            if (relevantActivities.length === 0) {
                ...
                if (lastActivity.type === 'stop') {
                    ...
                    const snoozeEnd = getZoneTime(new Date(lastActivity.timestamp)).getTime() + (snoozeTime * 60 * 1000);
...
export const getNearestUpcomingAlarm = (
    alarm_times: string[],
    activityHistory: AlarmActivity[],
    currentTime: Date // Note: caller should pass timezone corrected time or we convert it? 
    // Ideally caller passes the raw time and we convert, or caller passes converted time.
    // Let's assume caller might pass raw "now", so we ensure it's zoned.
) => {
    // Ensure currentTime is in zone if it isn't already (hard to detect, but safe to re-zone if it's raw UTC)
    // Actually, if caller uses `getCurrentTimeInZone()`, it's already shifted.
    // Let's assume input `currentTime` is already the "wall clock" time we want to compare against.
    ...
    for (const timeStr of alarm_times) {
        ...
        const alarmDate = getCurrentTimeInZone();
        // Sync date part with currentTime to be safe
        alarmDate.setFullYear(currentTime.getFullYear(), currentTime.getMonth(), currentTime.getDate());
        alarmDate.setHours(hours, minutes, 0, 0);
        ...
        const isHandled = activityHistory.some((activity: AlarmActivity) => {
            const activityTime = getZoneTime(new Date(activity.timestamp));
