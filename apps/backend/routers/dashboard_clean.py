...
from models.database_models import User, DashboardWidget, DailyWidget
...
async def get_today_widgets(
    ...
    """
    Get today's AI-curated dashboard widgets
    
    Returns widgets selected by AI for today's dashboard based on:
    ...
    try:
        ...
        # Get today's daily widgets with their dashboard widget details
        ...
        # If no daily widgets exist for today, generate them with AI
        if not daily_widgets:
            daily_widgets = await _generate_daily_widgets(db, user_id, target_date)
        
        # Format response with widget data
        widgets_data = []
